...
if __name__=="__main__":
    ...
    # confocal_depth = np.hstack([1.1, 4.2, 42.4, 'full'])
    confocal_depth = np.hstack([1.1, 2.1, 4.2, 8.5, 21.2, 42.4, 84.8, 217.2]) # plot this. 
    ...
    for imfile_ii in np.arange(len(imfiles))[:]:
        ...
        # binary_detection = np.logical_and(all_inds>threshold_final, all_n_peaks
        ...
        # binary_detection = skmorph.binary_closing(binary_detection, skmorph.disk(1)) # does make a diff --- i had this closing off. 
        ...
        """
        I think the bo-jui figure is of 3std and with no closing. 
        """
        
        # 2500 is much better 
        remove_areas = np.setdiff1d(np.unique(labelled_detection), 0)[np.hstack(areas) > 3*np.std(areas) + np.mean(areas)] # was 10000 
        # remove_areas = np.setdiff1d(np.unique(labelled_detection), 0)[np.hstack(areas) > 2*np.std(areas) + np.mean(areas)] # was 10000 
        # remove_areas = np.setdiff1d(np.unique(labelled_detection), 0)[np.hstack(areas) > 10000] #3*np.std(areas) + np.mean(areas)] # was 10000 
        # remove_areas = np.setdiff1d(np.unique(labelled_detection), 0)[np.hstack(areas) > 5000] # 2*np.std(areas) + np.mean(areas)] # was 10000 
        print('area_threshold', 3*np.std(areas) + np.mean(areas))
